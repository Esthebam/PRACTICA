...
class TodoList4Window extends SimpleWindow<PrincipalAppModel> {
	...
	override protected createFormPanel(Panel panel) {
		...
		new Button(panel) => [
			...
			onClick[this.editarTareaSeleccionada]
	...
	def editarTareaSeleccionada() {
		
		val tarea = modelObject.tareaSeleccionada
		new EditarTareaWindow(this, tarea) => [
			onAccept[this.modelObject.actualizarTareaSeleccionada]
			open
		]
	}
